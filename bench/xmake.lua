...
option("bench", { default = false, description = "Enable benchmark"})

if has_config("bench") then
	add_requires("nanobench")

	set_group("Benchmark")
	for _, file in pairs(os.files("src/*")) do
        target("bench-" .. path.basename(file), function ()
            add_files(file)
            add_deps("NazaraUtils")
            add_defines("ANKERL_NANOBENCH_IMPLEMENT")
